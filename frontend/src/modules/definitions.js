...
export const modules = {
  rrhh: {
    ...
    resources: [
      ...
      ], quickCreate: { label: 'Registrar asistencia', resource: 'control_asistencia', fields: [text('ci_empleado','CI del empleado', { placeholder: 'Ej: 8564321' }), select('id_empleado','Empleado','id_empleado')] } },
      ...
      // Contrato empleado (coincide con tu tabla)
      { 
        key: 'contrato_empleado',
        title: 'Contratos de empleado',
        id: 'id_contrato',
        fields: [
          select('id_empleado', 'Empleado', 'id_empleado'),
          select('id_tipo_contrato', 'Tipo contrato', 'id_tipo_contrato'),
          select('id_tipo_pago', 'Tipo pago', 'id_tipo_pago'),
          number('tarifa', 'Tarifa'),
          date('fecha_inicio', 'Fecha inicio'),
          date('fecha_fin', 'Fecha fin'),
          staticSelect('estado_contrato', 'Estado contrato', estadoContrato)
        ]
      },

      // Control asistencia
      { key: 'control_asistencia', title: 'Asistencia de trabajadores', id: 'id_asistencia', fields: [
        text('ci_empleado','CI del empleado', { placeholder: 'Ej: 8564321' }),
        select('id_empleado','Empleado','id_empleado'),
        date('fecha','Fecha'),
        time('hora_entrada','Hora entrada'),
        time('hora_salida','Hora salida'),
        number('horas_trabajadas','Horas trabajadas'),
        number('horas_extra','Horas extra'),
        staticSelect('estado_asistencia','Estado asistencia',asistencia)
      ] },

      // Resumen diario asistencia
      ...
      { key: 'descuentos_empleado', title: 'Descuentos aplicados', id: 'id_descuento', fields: [
        select('id_empleado','Empleado','id_empleado'),
        select('id_nomina_resumen','Nómina relacionada','id_nomina_resumen', { help: 'Opcional' }),
        staticSelect('tipo_descuento','Tipo descuento',['Falta','Retardo','Administrativo','Adelanto','Otro']),
        number('monto_descuento','Monto descuento'),
        date('fecha_descuento','Fecha del descuento'),
        area('motivo','Motivo del descuento')
      ] },

      // Pagos nómina (permitir editar si backend lo permite)
      { 
        key: 'nomina_pagos',
        title: 'Pagos de empleados / nómina',
        id: 'id_nomina',
        fields: [
          select('id_empleado','Empleado','id_empleado'),
          select('id_periodo_pago','Periodo pago','id_periodo_pago'),
          date('fecha_pago','Fecha pago'),
          date('periodo_inicio','Periodo inicio'),
          date('periodo_fin','Periodo fin'),
          number('dias_trabajados','Días trabajados'),
          number('horas_trabajadas','Horas trabajadas'),
          number('horas_extra','Horas extra'),
          number('monto_pago','Monto pago'),
          staticSelect('estado_pago','Estado pago',['Pendiente','Pagada','Anulada','Cancelada'])
        ]
      }
    ]
  },
  ...
  clientesFinanzas: {
    ...
    resources: [
      ...
      { key: 'pagos_cliente', title: 'Pagos del cliente', id: 'id_pago_cliente', paymentQR: true, fields: [select('id_cliente','Cliente','id_cliente'), select('id_proyecto','Proyecto','id_proyecto'), number('monto','Monto'), date('fecha_pago','Fecha pago'), staticSelect('metodo_pago','Método',metodosPago)] },
      { key: 'plan_pagos', title: 'Plan de pagos', id: 'id_plan_pago', fields: [select('id_proyecto','Proyecto','id_proyecto'), select('id_fase','Fase','id_fase', { dependsOn: 'id_proyecto', disabledOnCreate: true })] },
  ...
  proyectos: {
    ...
    resources: [
      ...
      { key: 'proyecto_empleado', title: 'Asignar empleados a proyectos', id: 'id_proyecto_empleado', fields: [select('id_proyecto','Proyecto','id_proyecto'), select('id_empleado','Empleado','id_empleado'), text('rol_en_proyecto','Rol')] },
      { key: 'proyecto_material', title: 'Asignación material a proyecto', id: 'id_proyecto_material', fields: [select('id_proyecto','Proyecto','id_proyecto', { required: true }), select('id_material','Material','id_material', { required: true })] }
    ]
  },
  ...
  inventario: {
    ...
    resources: [
      { key: 'materiales', title: 'Materiales', id: 'id_material', fields: [text('codigo_material','Código material', { disabled: true, help: 'Se genera automáticamente.' }), text('nombre_material','Nombre material'), area('descripcion_material','Descripción')] },
      ...
      { key: 'orden_pedido', title: 'Órdenes de pedido', id: 'id_orden', fields: [select('id_proveedor','Proveedor','id_proveedor', { required: true }), select('id_material','Material','id_material', { required: true }), number('cantidad','Cantidad'), date('fecha_pedido','Fecha pedido')] },
  ...
  proveedores: {
    ...
    resources: [
      { key: 'proveedores', title: 'Proveedores', id: 'id_proveedor', fields: [text('nombre_proveedor','Nombre proveedor', { required: true }), text('telefono_proveedor','Teléfono'), area('direccion_proveedor','Dirección')] },
      { key: 'pagos_proveedor', title: 'Pagos proveedor', id: 'id_pago_proveedor', fields: [select('id_proveedor','Proveedor','id_proveedor', { required: true }), select('id_orden','Orden de pedido','id_orden'), number('monto','Monto'), date('fecha_pago','Fecha pago')] }
    ]
  },
  ...
  subcontratistas: {
    ...
    resources: [
      ...
      { key: 'contrato_subcontratista', title: 'Contratos subcontratistas', id: 'id_contrato_sub', fields: [select('id_subcontratista','Subcontratista','id_subcontratista', { required: true }), select('id_proyecto','Proyecto','id_proyecto')] },
      { key: 'pago_subcontratista', title: 'Pagos subcontratistas', id: 'id_pago_sub', fields: [select('id_contrato_sub','Contrato','id_contrato_sub', { required: true }), number('monto','Monto pago'), date('fecha_pago','Fecha pago')] }
    ]
  },
  ...
  catalogos: {
    ...
    resources: [
      { key: 'cargo_empleado', title: 'Cargo empleado', id: 'id_cargo', fields: [text('nombre_cargo','Nombre del cargo', { required: true }), area('descripcion_cargo','Descripción'), number('nivel','Nivel')] },
  ...
  seguridad: {
    ...
    resources: [
      ...
      { key: 'intentos_login', title: 'Intentos de login / historial', id: 'id_intento', fields: [], readonly: true, lockMessage: 'Registro automático. Más de 3 intentos fallidos bloquean el intento.' },
