export const resources = {
  ...
  intentos_login: { table: 'intentos_login', id: 'id_intento', allowCreate: false, allowUpdate: false, allowDelete: false, select: `SELECT il.id_intento, u.username, il.id_usuario, CONVERT(VARCHAR(19), il.fecha_intento, 120) AS fecha_intento, il.exitoso, il.ip_origen, il.motivo_fallo FROM intentos_login il LEFT JOIN usuarios u ON u.id_usuario = il.id_usuario`, columns: [], search: ['id_intento','username','exitoso','ip_origen','motivo_fallo'] },
  auditoria: { table: 'auditoria', id: 'id_auditoria', allowCreate: false, allowUpdate: false, allowDelete: false, select: 'SELECT id_auditoria, tabla_afectada, operacion, usuario_nombre, CONVERT(VARCHAR(19), fecha, 120) AS fecha, datos_anteriores, datos_nuevos, id_usuario_auditor FROM auditoria', columns: [], search: ['id_auditoria','tabla_afectada','operacion','usuario_nombre'] },
  ...
  cotizaciones: {
    ...
    columns: ['presupuesto_cliente','id_cliente_cotizacion','ubicacion_obra','metros_cuadrados','numero_pisos','tiempo_estimado','costo_estimado_materiales','costo_estimado_mano_obra','otros_costos_estimados','fecha_cotizacion','estado_cotizacion','observaciones'],
  ...
  plan_pagos: {
    ...
    select: `SELECT pp.id_plan_pago, p.nombre_proyecto AS proyecto, fp.nombre_fase, pp.numero_cuota, pp.monto_esperado, pp.fecha_limite, pp.estado_pago, pp.porcentaje_asociado, pp.id_proyecto, pp.id_fase, p.id_cliente
      FROM plan_pagos pp
  ...
  liquidaciones: {
    ...
    select: `SELECT l.id_liquidacion, p.nombre_proyecto AS proyecto, CONCAT(c.nombre_cliente, ' ', ISNULL(c.apellido_cliente, '')) AS cliente,
      ...
      l.otros_costos_reales, l.monto_total_real, l.ganancia_real, l.observaciones
      FROM liquidaciones l
      LEFT JOIN proyectos p ON p.id_proyecto = l.id_proyecto
      LEFT JOIN cotizaciones co ON co.id_cotizacion = l.id_cotizacion
      LEFT JOIN clientes c ON c.id_cliente = co.id_cliente_cotizacion`,
    columns: ['id_proyecto','id_cotizacion','fecha_liquidacion','costo_real_materiales','costo_real_mano_obra','otros_costos_reales','monto_total_real','ganancia_real','observaciones'],
  ...
  proyectos: {
    ...
    select: `SELECT p.id_proyecto, p.codigo_proyecto, p.nombre_proyecto,
      ...
      co.id_cotizacion, p.ubicacion_proyecto, p.fecha_inicio_proyecto, p.fecha_fin_proyecto,
      p.costo_real_proyecto, p.prioridad_proyecto, p.porcentaje_avance, p.estado_registro,
      p.id_cliente, p.id_estado_proyecto, p.id_centro_costo
      FROM proyectos p
    ...
    columns: ['id_cliente','id_cotizacion','id_centro_costo','id_estado_proyecto','codigo_proyecto','nombre_proyecto','ubicacion_proyecto','fecha_inicio_proyecto','fecha_fin_proyecto','costo_real_proyecto','prioridad_proyecto','porcentaje_avance','estado_registro'],
    search: ['id_proyecto','codigo_proyecto','nombre_proyecto','cliente','estado_proyecto','centro_costo','prioridad_proyecto','estado_registro'],
  },
  fases_proyecto: {
  ...
  movimiento_inventario: {
    ...
    select: `SELECT mi.id_movimiento, m.nombre_material AS material, p.nombre_proyecto AS proyecto, mi.tipo_movimiento, mi.cantidad, mi.fecha, mi.id_material, mi.id_proyecto
  ...
  proyecto_material: {
    ...
    select: `SELECT pm.id_proyecto_material, p.nombre_proyecto AS proyecto, m.nombre_material AS material, f.nombre_fase,
    ...
    search: ['id_proyecto_material','proyecto','material','nombre_fase'],
  },
  costos_material: {
  ...
  pagos_proveedor: {
    ...
    select: `SELECT pp.id_pago_proveedor, pr.nombre_proveedor AS proveedor, p.nombre_proyecto AS proyecto,
      ep.estado_pago_proyecto AS estado_pago, pp.monto, pp.fecha_pago, pp.id_proveedor, pp.id_proyecto, pp.id_estado_pago
    ...
    columns: ['id_proveedor','id_proyecto','id_estado_pago','monto','fecha_pago'],
  ...
  pago_subcontratista: {
    ...
    select: `SELECT ps.id_pago_sub, s.nombre_subcontratista AS subcontratista, p.nombre_proyecto AS proyecto,
